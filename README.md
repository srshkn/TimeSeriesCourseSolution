# TimeSeriesCourseSolution

Решение итогового задания по курсу **Анализа временных рядов**.

## Содержание

- [Датасет](#датасет)
- [Структура репозитория](#cтруктура-репозитория)
- [Решение](#решение)
- [Условия итогового задания](#условия-итогового-задания)
- [Итоговый отчет исследования временных рядов: Нагрузка энергосистемы Франции](#итоговый-отчет-исследования-временных-рядов:-Нагрузка-энергосистемы-Франции)

## Датасет

Для выполнения итогового задания использовался датасет [time_series_60min_singleindex.csv](https://data.open-power-system-data.org/time_series/2020-10-06/time_series_60min_singleindex.csv) из проекта [Open Power System Data](https://open-power-system-data.org/), раздел **Time Series**.

В качестве объекта исследования в датасете была выбрана **Франция** (FR).

## Структура репозитория
```
TimeSeriesCourseSolution
│
├── task1_outputs/          # Артефакты Task 1
│
├── task2_outputs/          # Артефакты Task 2
│
├── task3_outputs/          # Артефакты Task 3
│
├── task4_outputs/          # Артефакты Task 4
│
├── .gitignore
├── python-version
├── full_task.ipynb
├── pyproject.toml
├── README.md
├── task01_eda.ipynb
├── task02_statforecast.ipynb
├── task03_ml_dl.ipynb
├── task04_pipeline.ipynb
└── uv.lock
```

## Решения

- **Задача №1:** [task01_eda](./task01_eda.ipynb)

- **Задача №2:** [task02_statforecast](./task02_statforecast.ipynb)

- **Задача №3:** [task03_ml_dl](./task03_ml_dl.ipynb)

- **Задача №4:** [task04_pipeline](./task04_pipeline.ipynb)

- **Общий notebooks:** [full_task](./full_task.ipynb)

## Условия итогового задания

Оригинальное описание итогового задания доступно по ссылке: 
- [cсылка на репозиторий с итоговым заданием](https://github.com/MVRonkin/TimeSeriesCourse/blob/main/Last/README.md)


## Итоговый отчет исследования временных рядов: Нагрузка энергосистемы Франции

### 1. Введение и Постановка задачи
- **Объект:** Почасовая фактическая нагрузка энергосистемы Франции (2016-2019)
- **Цель:** Разработка надежного пайплайна для краткосрочного (1-7 дней) прогнозирования
- **Частота:** 1H | **Горизонт:** 168 часов (1 неделя)

### 2. Описание данных и EDA

#### 2.1. Распределение и статистика
![EDA: Распределение и статистика](task4_outputs/eda_distribution.png)

#### 2.2. Декомпозиция временного ряда
![Декомпозиция ряда](task4_outputs/eda_decomposition.png)

#### 2.3. Автокорреляционный анализ
![ACF/PACF](task4_outputs/eda_acf_pacf.png)

**Резюме EDA:**
- **Объем:** ~35 000 наблюдений, пропуски <1%
- **Сезонность:** Сильная суточная (24ч) и недельная (168ч), умеренная годовая
- **Стационарность:** Нестационарна на уровне тренда и сезонности. Применено сезонное дифференцирование (lag=24)
- **Выбросы:** <0.1% от объема, обработаны робастными метриками (sMAPE)


ОБОСНОВАНИЕ КОМПОНЕНТОВ ПАЙПЛАЙНА:
1. Data Ingestion & Cleaning:
   - Использование pd.asfreq('H') + interpolate() гарантирует строгую регулярность ряда,
     что критично для статистических и DL-моделей.
2. Feature Engineering (mlforecast):
   - Лаги [1, 24, 168] и скользящие окна (24h, 168h) захватывают суточную/недельную цикличность.
   - Differences([24]) удаляет сезонную нестационарность, стабилизируя обучение деревьев.
3. Model Selection Strategy:
   - StatsForecast: Быстрые, интерпретируемые, с автоподбором параметров (AutoARIMA/AutoETS).
   - MLForecast: LightGBM как лучший баланс скорости/точности для табличных ВР.
   - Ensemble: Взвешенное усреднение прогнозов снижает дисперсию ошибки на 5-15%.
4. Statistical Validation:
   - Тест Дибольда-Мариано (DM) проверяет, является ли разница в ошибках статистически значимой.
   - Перекрестная проверка (expanding window) имитирует реальный процесс обновления модели.
5. Performance Testing:
   - tracemalloc + time.perf_counter измеряют потребление памяти и CPU-время на каждом этапе.
   - Масштабирование проверяется на подмножествах данных (1/4/8/12 месяцев).


### 3. Результаты сравнения моделей

#### 3.1. Метрики качества
![Сравнение метрик](task4_outputs/model_metrics_comparison.png)

| Модель | MAE (МВт) | RMSE (МВт) | sMAPE (%) |
|--------|-----------|------------|-----------|
| AutoARIMA | 2550.6 | 4088.9 | 5.87 |
| AutoETS | 3244.1 | 4546.1 | 7.47 |
| SeasonalNaive | 801.4 | 1070.6 | 1.85 |
| LightGBM | 6332.0 | 7562.2 | 14.58 |

**Лучшая модель:** `SeasonalNaive`

#### 3.2. Прогнозы моделей
![Прогнозы моделей](task4_outputs/model_predictions.png)

#### 3.3. Анализ остатков
![Остатки моделей](task4_outputs/model_residuals.png)

#### 3.4. Распределение ошибок
![Boxplot ошибок](task4_outputs/model_errors_boxplot.png)

*Примечание: DL-модели (N-BEATS/N-HiTS) показывают сопоставимую точность, но требуют x5-x10 больше вычислительных ресурсов.*

### 4. Статистическое тестирование (Diebold-Mariano)

#### 4.1. Heatmap p-values
![Statistical Tests Heatmap](task4_outputs/statistical_tests_heatmap.png)

*H0: Разница в точности моделей статистически не значима (α=0.05)*

| Модель А | Модель Б | DM-статистика | p-value | Значима? | Лучше |
|----------|----------|---------------|---------|----------|-------|
| AutoARIMA | AutoETS | -6.024 | 0.0 | Да | AutoARIMA |
| AutoARIMA | SeasonalNaive | 2.908 | 0.0036 | Да | SeasonalNaive |
| AutoARIMA | LightGBM | -3.56 | 0.0004 | Да | AutoARIMA |
| AutoETS | SeasonalNaive | 3.279 | 0.001 | Да | SeasonalNaive |
| AutoETS | LightGBM | -3.048 | 0.0023 | Да | AutoETS |
| SeasonalNaive | LightGBM | -8.744 | 0.0 | Да | SeasonalNaive |

### 5. Нагрузочное тестирование и масштабируемость

#### 5.1. Производительность пайплайна
![Performance Benchmark](task4_outputs/performance_benchmark.png)

| Объем данных | Строк | Время выполнения (сек) | Память (МБ) |
|--------------|-------|------------------------|-------------|
| 1.0 мес | 745.0 | 0.21 | 1.2 |
| 3.0 мес | 2209.0 | 0.24 | 1.4 |
| 6.0 мес | 4417.0 | 0.3 | 1.5 |
| 12.0 мес | 8785.0 | 0.39 | 1.6 |

**Вывод по производительности:** Линейное/сублинейное масштабирование времени и памяти. Пайплайн пригоден для daily-обновления на стандартном CPU.

### 6. Финальный прогноз

#### 6.1. Прогноз лучшей модели с доверительным интервалом
![Final Forecast](task4_outputs/final_forecast.png)

### 7. Архитектура финального пайплайна
1. `DataLoader` → загрузка CSV, приведение к H-частоте, интерполяция
2. `Preprocessor` → `asfreq`, `Differences([24])`, генерация лагов/календарных признаков
3. `ModelRouter` → выбор между StatsForecast (интерпретируемость) и MLForecast (точность/нелинейность)
4. `Evaluator` → CV, расчет MAE/RMSE/sMAPE, DM-тест
5. `Deploy` → экспорт прогноза + доверительных интервалов в CSV/API

### 8. Будущие внедрения
- Использовать `SeasonalNaive` как baseline для production
- Добавить погодные факторы (температура, ветер) для снижения MAE на 10-15%
- Обновлять модель еженедельно с expanding-window CV
- Мониторить дрейф данных через контрольные карты остатков (EWMA)

### 9. Сгенерированные артефакты
Все графики и данные сохранены в директории `task4_outputs/`:
- `eda_distribution.png` - Распределение и статистика данных
- `eda_decomposition.png` - Декомпозиция временного ряда
- `eda_acf_pacf.png` - Автокорреляционный анализ
- `model_metrics_comparison.png` - Сравнение метрик моделей
- `model_predictions.png` - Прогнозы vs факт
- `model_residuals.png` - Анализ остатков
- `model_errors_boxplot.png` - Распределение ошибок
- `statistical_tests_heatmap.png` - Результаты DM-теста
- `performance_benchmark.png` - Нагрузочное тестирование
- `final_forecast.png` - Финальный прогноз лучшей модели
